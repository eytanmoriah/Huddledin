...
#bottom-nav{display:none;position:fixed;bottom:0;left:0;right:0;z-index:200;background:rgba(255,255,255,.94);-webkit-backdrop-filter:saturate(200%) blur(28px);backdrop-filter:saturate(200%) blur(28px);border-top:1px solid rgba(0,0,0,.06);box-shadow:0 -1px 0 rgba(255,255,255,.8),0 -8px 32px rgba(0,0,0,.08);padding-bottom:var(--safe-bottom)}
...
.inp{width:100%;padding:14px 16px;border-radius:var(--r);border:1.5px solid var(--mint-l);font-family:'Nunito',sans-serif;font-size:16px;font-weight:600;outline:none;background:var(--white);color:var(--navy);transition:border-color .15s,box-shadow .15s;-webkit-appearance:none;appearance:none;min-height:48px}
...
.notif-item.unread{background:linear-gradient(135deg,var(--mint-ll),#fff)}
...
/* Drawer scroll area */
.drawer-scroll{flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch;padding:12px 10px}
...
.cal-fs-body{flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch}
...
/* ═══════════════════════════════════════════════
   ...
   In production, ALL API calls (Dropbox, Anthropic) MUST be
   proxied through a secure backend server. The tokens below
   should be rotated immediately if this file was shared publicly.

   ── HOW TO INJECT TOKENS ──────────────────────
   Add a <script> block in the <head> BEFORE this file loads,
   or have your server template render it inline:

     <script>
       window.DROPBOX_TOKEN  = "sl.YOUR_DROPBOX_ACCESS_TOKEN";
       window.ANTHROPIC_API_KEY = "sk-ant-YOUR_ANTHROPIC_KEY";
     <\/script>

   For Dropbox specifically:
     • App key / App secret are used ONLY in the OAuth flow
       (server-side) to exchange an authorization code for the
       access token. They should NEVER appear in client JS.
     • Generate a long-lived (offline) access token at
       https://www.dropbox.com/developers/apps → your app →
       OAuth 2 → Generated access token.
     • Assign that access token to window.DROPBOX_TOKEN above.

   For local development only (never commit this):
     Open the file in a text editor, find the <head> tag, and
     paste the <script> block above with your real token.
     Keep a .gitignore'd copy of the file with tokens injected,
     and always distribute the token-free version.
═══════════════════════════════════════════════ */
const DROPBOX_TOKEN = typeof window.DROPBOX_TOKEN !== 'undefined'
  ? window.DROPBOX_TOKEN
  : '';
...
const SB = {
  async getChildren(parentId){
    ...
    return (data||[]).map(c=>({id:c.id,name:c.name,age:c.dob?new Date().getFullYear()-new Date(c.dob).getFullYear():null,avatar:c.avatar_emoji||'🧒',color:c.color||'#0d9488',dob:c.dob,photo:c.photo_url}));
  },
  ...
  async getSpecialists(parentId){
    ...
    return (data||[]).map(s=>({id:s.id,name:s.name,role:s.role,email:s.email,avatar:'🩺',permissions:{},status:s.status,credentials:[],inviteToken:s.invite_token}));
  },
  ...
  async uploadFile(childId,file){
    const path=`${childId}/${Date.now()}_${file.name}`;
    const {error}=await _supa.storage.from('huddledin-files').upload(path,file,{contentType:file.type});
    if(error)throw error;
    const {data:signed}=await _supa.storage.from('huddledin-files').createSignedUrl(path,3600);
  ...
  try{
    if(session.role==='parent'){
      ...
      if(hid){
        ...
        children=(data||[]).map(c=>({id:c.id,name:c.name,age:c.dob?new Date().getFullYear()-new Date(c.dob).getFullYear():null,avatar:c.avatar_emoji||'🧒',color:c.color||'#0d9488',dob:c.dob,photo:c.photo_url}));
      } else {
      ...
      try{
        const {data:reqs}=await _supa.from('specialist_requests').select('*').eq('parent_id',session.id).eq('status','pending');
        if(reqs&&reqs.length>0){
          const mapped=reqs.map(r=>({id:r.id,specialistId:r.specialist_id,specialistName:r.specialist_name,role:r.role,childId:r.child_id,requestedAt:r.requested_at,status:r.status}));
          const existing=DB.specialistRequests||[];
          const merged=[...existing];
          mapped.forEach(r=>{if(!merged.some(e=>e.id===r.id))merged.push(r);});
          DB.specialistRequests=merged;
        }
      }catch(e){}
    } else if(session.role==='specialist'){
      const {data:specRows}=await _supa.from('specialists').select('child_id').eq('id',session.id).eq('status','active');
      if(specRows&&specRows.length>0){
        ...
        if(childIds.length>0){
          ...
          if(children&&children.length>0){
            const mapped=children.map(c=>({id:c.id,name:c.name,dob:c.dob,avatar:c.avatar_emoji||'🌸',color:c.color||'#c4b5fd',photo:c.photo_url||null}));
...
function setupHouseholdSync(){
  ...
  _householdChannel=_supa.channel('household:'+session.householdId)
    .on('postgres_changes',{event:'*',schema:'public',table:'children',filter:'household_id=eq.'+session.householdId},()=>{
      refreshFromSupabase().then(()=>re());
    })
    .on('postgres_changes',{event:'*',schema:'public',table:'specialists',filter:'household_id=eq.'+session.householdId},()=>{
      refreshFromSupabase().then(()=>re());
    })
    .subscribe();
}
...
async function _loadSessionFromSupabase(s){
  ...
  if(profile){
    session={id:s.user.id,email:s.user.email,role:profile.role,
      name:profile.display_name||s.user.email.split('@')[0],specialistId:profile.id,isNew:false,
    ...
    if(profile.role==='parent'){
      ...
      let children=[];
      if(hid){
        const {data:hChildren}=await _supa.from('children').select('*').eq('household_id',hid).order('created_at');
        children=(hChildren||[]).map(c=>({id:c.id,name:c.name,age:c.dob?new Date().getFullYear()-new Date(c.dob).getFullYear():null,avatar:c.avatar_emoji||'🧒',color:c.color||'#0d9488',dob:c.dob,photo:c.photo_url}));
      } else {
        children=await SB.getChildren(s.user.id);
      }
      LS.set('children',children);
      if(!S.activeChild&&children.length>0)S.activeChild=children[0].id;
      // Load specialists by household_id
      try{
        const specs=hid
          ? await SB.getSpecialistsByHousehold(hid)
          : await SB.getSpecialists(s.user.id);
        if(specs&&specs.length>0)LS.set('specialists',specs);
      }catch(e){}
...
(async()=>{
  ...
  _supa.auth.onAuthStateChange(async(event,s2)=>{
    if(event==='SIGNED_IN'&&s2&&!session){
      ...
      console.log('onAuthStateChange SIGNED_IN, pendingInvite:',_pi,'session:',session?.id);
  ...
  if(session){refreshFromSupabase().then(()=>{re();setupHouseholdSync();});}
})();
...
async function doLogin(email,password,role,name='',profession=''){
  ...
  if(error&&error.message.includes('Invalid')){
    ...
    if(data.user){try{await _supa.from('profiles').upsert({id:data.user.id,email,role,display_name:name||email.split('@')[0],profession:profession||null},{onConflict:'id',ignoreDuplicates:true});}catch(e){}}
...
async function redeemConnectionCode(code,specialistId,specialistName,role){
  ...
  if(session&&session.id!=='demo'){
    ...
    if(row){
      ...
      try{
        await _supa.from('specialist_requests').upsert({
        ...
        },{onConflict:'specialist_id,child_id'});
      }catch(e){console.log('specialist_requests upsert error:',e);}
      // Also add to specialists table so specialist can see the child
      try{
...
function approveSpecialistRequest(requestId,selectedFolders){
  ...
  pushNotif('invite',`You approved ${req.specialistName} to access ${child?.name||'your child'}'s folders.`,req.childId);
  re();
  toast('Access granted to '+req.specialistName+'!');
}

function denySpecialistRequest(requestId){
  const requests=DB.specialistRequests||[];
...
function pushNotif(type,msg,childId){
  const ns=DB.notifications;
  ns.unshift({id:'n_'+Date.now(),type,msg,time:'Just now',read:false,childId});
  DB.notifications=ns;
  // Also fire browser push if permission granted
  ...
  function buildForm(){
    ...
    if(mode==='signup'){
      ...
      if(selRole==='specialist'){
        const pi2=el('input',{class:'inp',placeholder:'Your profession (e.g. Speech Therapist)',type:'text'});pi2.id='auth-profession';ig.appendChild(pi2);
      }
    }
    ...
    const sb=mkBtn(mode==='signin'?'Sign In ✨':'Create Account 🎉','btn-lg btn-primary btn-full',async()=>{
      ...
      const _pi=localStorage.getItem('pendingInviteToken');
      console.log('Email login, pendingInvite:',_pi,'session:',session?.id);
...
function renderHeader(){
  ...
  if(session?.role==='parent' && S.navLevel===1){
    ...
    logo.onclick=()=>{S.navLevel=0;S.isDrawerOpen=false;re();};
    ...
    if(child){
      ...
      const av=el('div',{style:{
        ...
        background:child.color+'44',display:'flex',alignItems:'center',
      ...
      if(allChildren.length>1){
        ...
        centre.onclick=e=>{
          ...
          allChildren.forEach(ch=>{
            ...
            const av2=el('div',{style:{width:'32px',height:'32px',borderRadius:'50%',background:ch.color+'44',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'1.1rem',flexShrink:0}});
            ...
            item.onclick=ev=>{ev.stopPropagation();S.activeChild=ch.id;csDd.remove();csDd=null;re();};
            csDd.appendChild(item);
          });
          // Position relative to header
          document.querySelector('.app-header')?.appendChild(csDd);
    ...
    const spec=DB.specialists.find(s=>s.id===session?.specialistId);
...
function renderNotifBell(){
  ...
  btn2.onclick=e=>{
    ...
    const dh=el('div',{class:'notif-dd-hd'},[
      ...
      mkBtn('See all','btn-sm btn-lav',()=>{dd.remove();dd=null;S.activeTab='notifications';re();})
    ]);
    dd.appendChild(dh);
    (notifs.slice(0,6)).forEach(n=>{
      const ni=el('div',{class:'notif-item'+(n.read?'':" unread")});
      ni.innerHTML=`<span style="font-size:1.25rem">${notifIcon[n.type]||'📢'}</span><div><div style="font-size:.79rem;font-weight:700;color:var(--navy);line-height:1.4">${n.msg}</div><div style="font-size:.68rem;color:var(--slate-l);margin-top:2px">${n.time}</div></div>`;
      ni.onclick=()=>{
        const ns=DB.notifications,i=ns.findIndex(x=>x.id===n.id);
        if(i>-1){ns[i].read=true;DB.notifications=ns;}
        dd.remove();dd=null;S.activeTab='notifications';re();
      };
      dd.appendChild(ni);
    });
    wrap.appendChild(dd);
...
function renderBottomNav(){
  const unreadNotifs=DB.notifications.filter(n=>!n.read).length;
  const unreadEmails=(DB.emailInbox||[]).filter(e=>!e.read).length;
  const totalBadge=unreadNotifs+unreadEmails;

  // Quick tabs depend on level and role
  let quickTabs;
  if(session?.role==='parent'){
    if(S.navLevel===0){
      quickTabs=[
        {id:'home',label:'Home',icon:'🏠',action:()=>{S.navLevel=0;re();}},
        {id:'calendar-global',label:'Calendar',icon:'📅',action:()=>{S.calFullscreen=true;re();}},
        {id:'chat-global',label:'Chat',icon:'💬',action:()=>{S.navLevel=1;switchTab('chat');}},
      ];
    } else {
      quickTabs=[
        {id:'home',label:'Home',icon:'🏠',action:()=>{S.navLevel=0;re();}},
        {id:'chat',label:'Chat',icon:'💬',action:()=>switchTab('chat')},
        {id:'files',label:'Files',icon:'📁',action:()=>switchTab('files')},
      ];
    }
  } else {
    quickTabs=[
  ...
  quickTabs.forEach(t=>{
    const isActive=t.id==='home'?(S.navLevel===0&&session?.role==='parent'):(S.activeTab===t.id&&S.navLevel===1)||(t.id==='calendar-global'&&S.calFullscreen);
...
function renderSearchOverlay(){
  ...
  const doSearch=q=>{
    ...
    DB.children.filter(c=>c.name.toLowerCase().includes(q.toLowerCase())).forEach(c=>{
      ...
      item.innerHTML=`<span style="font-size:1.5rem">${c.avatar}</span><div><div style="font-weight:700;color:var(--navy)">${c.name}</div><div style="font-size:.76rem;color:var(--slate)">Patient · Age ${c.age}</div></div>`;
    ...
    DB.vaultNotes.filter(n=>{
      ...
      const item=el('div',{class:'search-result-item'});
      item.innerHTML=`<span style="font-size:1.5rem">📝</span><div><div style="font-weight:700;color:var(--navy)">${n.title}</div><div style="font-size:.76rem;color:var(--slate)">${c?.avatar} ${c?.name} · ${n.date}</div></div>`;
      item.onclick=()=>{S.activeChild=n.childId;S.activeTab='vault';S.activeVaultId=n.id;S.searchOpen=false;S.searchQuery='';re();};
      results.appendChild(item);
    });
...
function renderGlobalHome(){
  ...
  children.forEach((c,idx)=>{
    ...
    const circle=el('div',{class:'cb-circle'+(c.photo?'':' placeholder'),style:{background:color+'18'}});
    if(c.photo){
    ...
    bubble.onclick=()=>{S.activeChild=c.id;S.navLevel=1;S.activeTab='dashboard';re();};
...
function renderMiniCalendar(children,allApts){
  ...
  expandBtn.onclick=e=>{e.stopPropagation();S.calFullscreen=true;re();};
  ...
  days.forEach(ds=>{
    ...
    col.onclick=()=>showAddEventModalForDate(ds,children);
  ...
  if(upcoming.length>0){
    ...
    upcoming.slice(0,4).forEach(a=>{
      ...
      const info=el('div',{style:{flex:1}});
      info.appendChild(el('div',{class:'mce-title'},[a.title]));
  ...
  card.onclick=()=>{S.calFullscreen=true;re();};
  return card;
}
...
function renderCalFullscreen(){
  ...
  const allApts=DB.appointments;
  const now=new Date();
  // FIX B4: use current month/year dynamically instead of hardcoded March 2025
  ...
  // Header
  const hd=el('div',{class:'cal-fs-hd'});
  const closeBtn=mkBtn('‹ Back','btn-sm btn-slate',()=>{S.calFullscreen=false;re();});
  hd.appendChild(closeBtn);
  // FIX B4: dynamic month label
  const calFsMonthLabel=now.toLocaleDateString('en-US',{month:'long',year:'numeric'})+' — All Children';
  hd.appendChild(el('span',{class:'ff',style:{fontWeight:800,fontSize:'1rem',color:'var(--navy)',flex:1,textAlign:'center'}},[calFsMonthLabel]));
  // Add Event button for parents (Task 5 — editable calendar)
  if(session?.role==='parent'){
    const addEvtBtn=mkBtn('+ Add','btn-sm btn-primary',()=>{
      // If no child is selected, use the first available child
      if(!S.activeChild&&children.length>0)S.activeChild=children[0].id;
      if(!S.activeChild){toast('Add a child profile first.','error');return;}
      showAddEventModal();
    });
  ...
  for(let day=1;day<=daysInMonth;day++){
    // FIX B4: build date string dynamically from current year/month
    const ds=`${year}-${String(month+1).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
    const dayApts=allApts.filter(a=>a.date===ds);
    const cell=el('div',{class:'cal-day'});
    const num=el('div',{class:'cal-num'+(dayApts.length?' has-event':'')},
      [String(day)]);
    if(dayApts.length>0){
      // Use first child's color for background
      const ci=children.findIndex(c=>c.id===dayApts[0].childId);
      const color=children[ci]?.color||getChildColor(ci);
      if(dayApts.length===1){num.style.background=`linear-gradient(135deg,${color},${color}bb)`;}
    }
    if(dayApts.length){
      num.onclick=()=>{
        const aptNames=dayApts.map(a=>{
          const c=children.find(x=>x.id===a.childId);
          return `${c?.avatar||''} ${a.title} (${c?.name.split(' ')[0]||''}) · ${a.time}`;
        }).join('\n');
        // FIX B4: use dynamic month label
        const aptMonthLabel=now.toLocaleDateString('en-US',{month:'long'})+' '+day;
        alert('📅 '+aptMonthLabel+':\n\n'+aptNames);
      };
    }
    ...
    // Multi-child dots
    if(dayApts.length>1){
      const dotsRow=el('div',{class:'cal-dots-row',style:{display:'flex',gap:'2px',justifyContent:'center',marginTop:'2px'}});
      const seen=new Set();
      dayApts.forEach(a=>{
        if(seen.has(a.childId))return;seen.add(a.childId);
        const ci=children.findIndex(c=>c.id===a.childId);
        dotsRow.appendChild(el('div',{class:'cal-dot',style:{background:children[ci]?.color||getChildColor(ci)}}));
      });
  ...
  if(upcoming.length>0){
    ...
    upcoming.forEach(a=>{
      ...
      const child=children[ci];
      const color=child?.color||getChildColor(ci);
      const card=el('div',{class:'card',style:{display:'flex',alignItems:'center',gap:'12px',marginBottom:'9px',padding:'13px 16px'}});
      card.appendChild(el('div',{style:{width:'5px',minHeight:'44px',borderRadius:'99px',background:color,flexShrink:0}}));
      const info=el('div',{style:{flex:1}});
      info.appendChild(el('div',{style:{fontWeight:800,color:'var(--navy)',fontSize:'.88rem'}},[a.title]));
      info.appendChild(el('div',{style:{fontSize:'.76rem',color:'var(--teal)',fontWeight:700,marginTop:'2px'}},[a.date+' · '+a.time]));
      card.appendChild(info);
      if(child){
        card.appendChild(el('span',{class:'badge',style:{background:color+'22',color}},
          [child.avatar+' '+child.name.split(' ')[0]]));
      }
      sec.appendChild(card);
    });
...
function renderParentDashboard(){
  ...
  if(!child){
    ...
    sec.appendChild(el('div',{class:'empty-state'},[
      ...
      el('div',{class:'empty-state-actions'},[
        mkBtn('← Family Home','btn-md btn-primary',()=>{S.navLevel=0;re();})
      ])
    ]));
    return sec;
  }
  ...
  const specialists=DB.specialists.filter(s=>s.status==='active');
  const childSpecs=specialists.filter(s=>s.permissions[child.id]?.length>0);
  ...
  [
    ['📈 Progress',()=>{S.activeTab='progress';re();}],
    ['📁 Files',()=>{S.activeTab='files';re();}],
    ['🔗 Invite Specialist',()=>showConnectionCodeModal(child.id)]
  ].forEach(([label,fn])=>{
  ...
  [{icon:'📅',color:'#0d9488',val:String(childApts.filter(a=>a.date>=new Date().toISOString().split('T')[0]).length),lbl:'Upcoming Apts',tab:'calendar'},
   {icon:'📋',color:'#f97316',val:String(childHW.filter(h=>!h.completed).length),lbl:'Pending Tasks',tab:'homework'},
   {icon:'👥',color:'#7c3aed',val:String(childSpecs.length),lbl:'Specialists',tab:'team'}
  ].forEach(({icon,color,val,lbl,tab})=>{
    const sc=el('div',{class:'card hov'});
    sc.appendChild(d('stat-card',[d('stat-icon',[icon],{background:color+'22'}),d('',[el('div',{class:'stat-val'},[val]),el('div',{class:'stat-lbl'},[lbl])])]));
  ...
  if(!hasAnyData){
    sec.appendChild(el('div',{class:'empty-state'},[
      ...
      el('div',{class:'empty-state-actions'},[
        ...
        mkBtn('📅 Add Appointment','btn-md btn-secondary',()=>{S.activeTab='calendar';re();})
      ])
    ]));
    return sec;
  }
  ...
  if(!childHW.length){
    ...
    childHW.slice(0,3).forEach(hw=>{
      ...
      chk.onclick=()=>{
        ...
        if(i>-1){all[i].completed=!all[i].completed;DB.homework=all;
          if(!hw.completed)pushNotif('homework',`"${hw.task.slice(0,38)}…" marked complete`,child.id);}
        re();
      };
  ...
  if(!childApts.length){
    ...
    childApts.slice(0,3).forEach(apt=>{
      ...
      const info=el('div',{style:{flex:1}});
      info.appendChild(el('div',{style:{fontWeight:700,fontSize:'.83rem',color:'var(--navy)'}},[apt.title]));
...
function renderSpecDashboard(){
  ...
  welcome.innerHTML=`<div style="color:rgba(255,255,255,.7);font-size:.78rem;font-weight:700;text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Welcome back</div>
    <div class="ff" style="font-size:1.45rem;font-weight:800;color:#fff;margin-bottom:5px">${spec?.name||session?.name||'Specialist'} 👋</div>
    <div style="color:rgba(255,255,255,.8);font-size:.84rem;font-weight:500">${spec?.role||'Specialist'} · <strong style="color:#fff">${myPatients.length}</strong> patient${myPatients.length!==1?'s':''}</div>`;
  const wBtns=el('div',{style:{display:'flex',gap:'8px',marginTop:'14px',flexWrap:'wrap'}});
  [
  ...
  [{icon:'👤',color:'#0d9488',lbl:'Patients',tab:'patients'},
   ...
   {icon:'📋',color:'#f59e0b',lbl:'Clinical Notes',tab:'vault'}
    ...
    sc.appendChild(d('stat-card',[d('stat-icon',[icon],{background:color+'22'}),d('',[el('div',{class:'stat-val',style:{fontSize:'1.5rem'}},[icon]),el('div',{class:'stat-lbl'},[lbl])])]));
    sc.onclick=()=>{S.activeTab=tab;re();};
    stats.appendChild(sc);
  });
  sec.appendChild(stats);
...
function renderFiles(){
  ...
  const spec=DB.specialists.find(s=>s.id===session?.specialistId);
  const perms=spec?.permissions[S.activeChild]||[];
  // Task 4 — vault-aware permission check
  // Standard folders: parent always access, specialist needs permission key
  ...
  if(isParent){
    // Regular file upload (desktop + file picker)
    ...
    hd.appendChild(el('div',{class:'sec-hd-right'},[
      mkBtn('📤 Upload File','btn-md btn-primary',()=>document.getElementById('fileInput')?.click())
    ]));
  }
  sec.appendChild(hd);
    ...
    if(!acc){
      ...
      const showLongPressMenu=(e)=>{
        ...
        const actions=[
          ...
          {icon:'ℹ️',label:`${folder.files.length} files`,fn:null},
        ];
        actions.forEach(a=>{
          const item=el('div',{class:'long-press-item'},['  ',a.icon,' ',a.label]);
      ...
      // Tap = open
      wrap.onclick=()=>{if(!moved)showFolderModal(folder,isParent);};
  ...
  if(S.activeChild){
    ...
    if(vaults.length>0){
      ...
      vaults.forEach(vault=>{
        ...
        const vaultSpec=DB.specialists.find(s=>s.id===vault.specialistId);
        ...
        if(canViewVault){
          ...
          wrap.onclick=()=>openModal('🔐 '+vaultSpec?.name+"'s Vault",(mb,close)=>{
            ...
            const otherSpecs=(DB.specialists||[]).filter(s=>s.status==='active'&&s.id!==vault.specialistId&&(s.permissions[vault.childId]||[]).length>0);
...
function showShareModal(file){
  openModal('📤 Upload & Share',(mb,close)=>{
    mb.appendChild(el('p',{style:{color:'var(--slate)',marginBottom:'16px',fontWeight:600}},[`Who can view "${file.name}"?`]));
    const permsMap={};
    const rows=el('div',{class:'inp-group',style:{marginBottom:'18px'}});
    DB.specialists.filter(s=>s.status==='active').forEach(s=>{
      permsMap[s.id]=false;
    ...
    DB.folders.forEach(f=>{const o=el('option');o.value=f.id;o.textContent=f.icon+' '+f.name;folderSel.appendChild(o);});
    mb.appendChild(folderSel);
    ...
    const upBtn=mkBtn('','btn-md btn-primary btn-full',async()=>{
      ...
      try{
        ...
        const child=DB.children.find(c=>c.id===S.activeChild);
        // FIX FS1: sanitize path segments to remove characters that are invalid in Dropbox paths
        const sanitizePath=s=>(s||'unknown').replace(/[\\/:*?"<>|]/g,'_').trim()||'unnamed';
        const safeChildName=sanitizePath(child?.name);
        const safeFileName=sanitizePath(file.name);
        const resp=await fetch('https://content.dropboxapi.com/2/files/upload',{
          method:'POST',
          headers:{Authorization:'Bearer '+DROPBOX_TOKEN,'Content-Type':'application/octet-stream',
            'Dropbox-API-Arg':JSON.stringify({path:`/Huddledin/${safeChildName}/${safeFileName}`,mode:'add',autorename:true})},body:ab
        });
        const data=await resp.json();
        const fls=DB.folders;
        const fi2=fls.find(f=>f.id===folderSel.value)||fls[0];
        fi2.files.push({id:'fl_'+Date.now(),name:data.name||file.name,date:new Date().toISOString().split('T')[0],locked:false,sharedWith:Object.entries(permsMap).filter(([,v])=>v).map(([k])=>k),uploadedBy:'parent'});
        DB.folders=fls;
        close();toast('✅ "'+( data.name||file.name)+'" uploaded to Dropbox!');track('File Uploaded');
      }catch(e){close();toast('❌ Upload failed: '+e.message,'error');}
    });
    upBtn.innerHTML='📤 Upload & Share';
...
function showFolderModal(folder,isParent){
  openModal(folder.icon+' '+folder.name,(mb,close)=>{
    ...
    folder.files.forEach(f=>{
      ...
      fi.innerHTML=`<span style="font-size:1.35rem">📄</span><div style="flex:1"><div style="font-weight:700;font-size:.84rem;color:var(--navy)">${f.name}</div><div style="font-size:.71rem;color:var(--slate);margin-top:2px">${f.date}</div></div>`;
      if(f.locked)fi.appendChild(el('span',{class:'locked-badge'},['🔒 Locked']));
      // Per-file share management (parent only)
      if(isParent&&!f.locked){
        const mgBtn=mkBtn('⚙️ Manage','btn-sm btn-slate',()=>showFilePermModal(f,folder));
        fi.appendChild(mgBtn);
      }
...
function showAccessRequest(folder){
  openModal('🔒 Request Access',(mb,close)=>{
    mb.appendChild(el('p',{style:{color:'var(--slate)',marginBottom:'20px',lineHeight:'1.65'}},
      [`You don't have access to ${folder.name}. Send a request to the parent?`]));
    mb.appendChild(mkBtn('📨 Send Request','btn-lg btn-primary btn-full',()=>{
      close();toast('✅ Access request sent to parent!');
    }));
  },400);
}
...
function renderTeam(){
  ...
  const hd=el('div',{class:'sec-hd'});
  hd.appendChild(el('div',{class:'sec-hd-left'},[
    el('h2',{class:'page-title'},['Care Team']),
    el('p',{class:'page-sub'},[child?`${child.avatar} ${child.name}'s specialists`:'No child selected'])
  ]));

  // Two invite actions
  const actRow=el('div',{class:'sec-hd-right',style:{display:'flex',gap:'8px',flexWrap:'wrap'}});
  actRow.appendChild(mkBtn('🔗 Share Code','btn-md btn-primary',()=>{
    if(!child){toast('Select a child first.','error');return;}
    showConnectionCodeModal(child.id);
  }));
  actRow.appendChild(mkBtn('📧 Email Invite','btn-md btn-secondary',()=>showInviteModal()));
  ...
  if(child){
    ...
    ci.appendChild(el('div',{style:{fontWeight:800,color:'var(--navy)',fontSize:'.88rem',marginBottom:'2px'}},
      ['Connection Codes — the fastest way to invite']));
    ci.appendChild(el('div',{style:{fontSize:'.76rem',color:'var(--slate)',fontWeight:500}},
      ['Generate a 6-character code and share it with a therapist. They enter it in their app to instantly join.']));
    connBanner.appendChild(ci);
    connBanner.appendChild(mkBtn('Generate','btn-sm btn-primary',()=>showConnectionCodeModal(child.id)));
    sec.appendChild(connBanner);
  }

  // Pending access requests (opt-in approval section)
  if(child){
    const pendingReqs=(DB.specialistRequests||[]).filter(r=>r.childId===child.id&&r.status==='pending');
    if(pendingReqs.length>0){
      sec.appendChild(el('h3',{style:{fontWeight:800,color:'var(--navy)',marginBottom:'12px',marginTop:'4px'}},['Pending Access Requests']));
      pendingReqs.forEach(req=>{
        ...
        // Folder selection for approval
        reqCard.appendChild(el('div',{class:'inp-label',style:{marginBottom:'8px'}},['Choose which folders to grant access:']));
        const folderKeys=['speech','ot','pt','general','meds','school'];
        const selFolders=[];
        ...
        folderKeys.forEach(key=>{
          ...
          const cb=el('input',{type:'checkbox'});
          cb.onchange=e=>{if(e.target.checked){if(!selFolders.includes(key))selFolders.push(key);}else{const fi=selFolders.indexOf(key);if(fi>-1)selFolders.splice(fi,1);}};
          lbl.appendChild(cb);lbl.appendChild(document.createTextNode((folder?.icon||'📁')+' '+(folder?.name||key)));
        ...
        btnRow.appendChild(mkBtn('Approve & Grant Access','btn-sm btn-primary',()=>{
          if(!selFolders.length){toast('Select at least one folder to grant.','error');return;}
  ...
  const specialists=DB.specialists.filter(s=>
    child?(s.permissions[child.id]?.length>0||s.status==='pending'):true
  );
  ...
  if(!specialists.length){
    ...
    specialists.forEach(s=>{
      ...
      perms.forEach(p2=>pr.appendChild(badge(p2.toUpperCase(),'teal')));
      if(!perms.length&&!isPending)pr.appendChild(badge('No access for this child','slate'));
      info.appendChild(pr);
      card.appendChild(info);
      if(!isPending&&child){
  ...
  if(archive.length){
    ...
    archive.forEach(a=>{
      ...
      ac.innerHTML=`<span style="font-size:1.4rem">🗄️</span><div style="flex:1"><div style="font-weight:700;color:var(--slate)">${a.specialistName}</div><div style="font-size:.76rem;color:var(--slate-l)">${a.childName} · Archived ${a.archivedAt} · ${a.notes.length} note(s) retained</div></div>`;
...
function showPermModal(spec){
  openModal('⚙️ Permissions — '+spec.name,(mb,close)=>{
    ...
    folderKeys.forEach(key=>{
      ...
      lbl.appendChild(el('span',{style:{fontWeight:700,fontSize:'.86rem'}},[folder?.name||key]));
      rows.appendChild(lbl);
    });
    mb.appendChild(rows);
    mb.appendChild(mkBtn('✅ Save Permissions','btn-lg btn-primary btn-full',()=>{
...
function showInviteModal(){
  openModal('➕ Invite a Specialist',(mb,close)=>{
    ...
    const ri=el('input',{class:'inp',placeholder:'Role (e.g. Speech Therapist)'});
    ...
    mb.appendChild(mkBtn('📨 Send Invitation','btn-lg btn-primary btn-full',()=>{
      ...
      const token='hd_invite_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,8);
      const child=DB.children.find(c=>c.id===S.activeChild);
      ...
      if(session&&session.id!=='demo'){
        ...
        console.log('Attempting invite insert, session.id:',session?.id,'child:',S.activeChild);
        _supa.from('invites').insert({
          ...
          accepted:false
        }).then(({data,error})=>{if(error){console.error('Invite save error DETAILS:',JSON.stringify(error));console.error('Code:',error.code,'Message:',error.message,'Details:',error.details);}else{console.log('Invite saved to Supabase:',data);}});
      }
      ...
      fetch('/api/send-invite',{
        ...
        console.log('Email API response:',JSON.stringify(d));
        if(d.ok)toast('📧 Invite email sent to '+ei.value+'!','success');
        else console.error('Email send failed:',d);
      }).catch(e=>console.error('Email API error:',e));
      toast('✅ Invitation created for '+ei.value+'! Magic link generated.');
...
function showHandshakeModal(spec){
  openModal('🤝 Consultation Request',(mb,close)=>{
    ...
    row2.appendChild(mkBtn('✓ Approve','btn-md btn-primary',()=>{
      ...
      if(other){
        const newChat={id:'ch_'+Date.now(),type:'specialist-consult',
          participants:inclCb.checked?['parent',spec.id,selectedConsult]:[spec.id,selectedConsult],
...
function renderCalendar(){
  ...
  const childApts=isParent
    ?DB.appointments.filter(a=>a.childId===S.activeChild)
    :DB.appointments.filter(a=>a.specialistId===(session?.specialistId)||a.sharedWith?.includes(session?.specialistId));
  const child=DB.children.find(c=>c.id===S.activeChild);
  ...
  hd.appendChild(el('div',{class:'sec-hd-left'},[
    ...
    // FIX B4: dynamic month label in calendar subtitle
    el('p',{class:'page-sub'},[isParent?`${child?.avatar} ${child?.name} — ${new Date().toLocaleDateString('en-US',{month:'long',year:'numeric'})}`:`My Schedule — ${new Date().toLocaleDateString('en-US',{month:'long',year:'numeric'})}`])
  ]));
  hd.appendChild(el('div',{class:'sec-hd-right'},[
  ...
  for(let day=1;day<=calDaysInMonth;day++){
      ...
      openModal('📅 '+evtLabel,(mb,close)=>{
        if(da.length){
          da.forEach(apt=>{
            ...
            chip.innerHTML=`<div style="font-weight:800;color:var(--navy);margin-bottom:4px">${apt.title}</div><div style="font-size:.8rem;color:var(--slate)">${apt.specialist} · ${apt.time}</div>`;
            mb.appendChild(chip);
          });
    ...
    cell.appendChild(num);
    da.forEach(a=>{
      const dot=el('div',{class:'cal-dot',style:{background:TC[a.type]||'var(--teal)'}},[a.title.split(' ')[0]]);
  ...
  const stack=el('div',{class:'stack'});
  [...childApts].sort((a,b)=>a.date.localeCompare(b.date)).forEach(apt=>{
    ...
    const bar=el('div',{style:{width:'5px',height:'48px',borderRadius:'3px',background:TC[apt.type]||'var(--teal)',flexShrink:0}});
    card.appendChild(bar);
    const info=d('',[], {flex:1});
    info.appendChild(el('div',{style:{fontWeight:800,color:'var(--navy)'}},[apt.title]));
    info.appendChild(el('div',{style:{fontSize:'.78rem',color:'var(--slate)',margin:'2px 0'}},[apt.specialist]));
    if(ch&&isParent)info.appendChild(el('div',{style:{fontSize:'.73rem',color:'var(--slate-l)'}},[ch.avatar+' '+ch.name]));
    card.appendChild(info);
    ...
    if(isParent){
      card.appendChild(mkBtn('🗑️','btn-sm btn-danger',()=>openConfirm('Delete Appointment','Remove this appointment?',true,()=>{
        const apts=DB.appointments;DB.appointments=apts.filter(a=>a.id!==apt.id);re();toast('✅ Appointment removed.');
      })));
    }
    stack.appendChild(card);
  });
...
function showAddEventModalForDate(dateStr,availableChildren){
  ...
  openModal('📅 Add Appointment',(mb,close)=>{
    mb.appendChild(el('div',{class:'inp-label'},['Which child?']));
    const childSel=el('select',{class:'inp',style:{marginBottom:'14px'}});
    theChildren.forEach(c=>{
      const opt=el('option',{value:c.id},[c.avatar+' '+c.name]);
      childSel.appendChild(opt);
    });
    if(S.activeChild&&theChildren.find(c=>c.id===S.activeChild))childSel.value=S.activeChild;
    mb.appendChild(childSel);
    mb.appendChild(el('div',{class:'inp-label'},['Title']));
    const titleInp=el('input',{class:'inp',placeholder:'e.g. Speech Therapy',style:{marginBottom:'14px'}});
    mb.appendChild(titleInp);
    mb.appendChild(el('div',{class:'inp-label'},['Date']));
    const dateInp=el('input',{class:'inp',type:'date',placeholder:'DD/MM/YYYY','data-placeholder':'Select date',style:{marginBottom:'14px'}});
    dateInp.value=dateStr||new Date().toISOString().split('T')[0];
    mb.appendChild(dateInp);
    mb.appendChild(el('div',{class:'inp-label'},['Time']));
    const timeInp=el('input',{class:'inp',type:'time',style:{marginBottom:'14px'}});
    timeInp.value='09:00';
    mb.appendChild(timeInp);
    mb.appendChild(el('div',{class:'inp-label'},['Notes (optional)']));
    const notesInp=el('input',{class:'inp',placeholder:'Any extra details',style:{marginBottom:'20px'}});
    mb.appendChild(notesInp);
    mb.appendChild(mkBtn('Add Appointment','btn-lg btn-primary btn-full',()=>{
      if(!titleInp.value.trim()){toast('Please enter a title.','error');return;}
      if(!dateInp.value){toast('Please enter a date.','error');return;}
      const apts=DB.appointments;
      apts.push({id:'a_'+Date.now(),childId:childSel.value,title:titleInp.value.trim(),
        date:dateInp.value,time:timeInp.value||'09:00',notes:notesInp.value.trim(),
        specialistId:session?.specialistId||null,sharedWith:[]});
      DB.appointments=apts;
      close();toast('Appointment added!','success');re();
    }));
  });
}
...
function showAddEventModal(prefilledDate=null){
  openModal('➕ Add Appointment',(mb,close)=>{
    const ig=el('div',{class:'inp-group',style:{marginBottom:'16px'}});
    // Child selector — shown when called from multi-child fullscreen calendar
    const children=DB.children;
    ...
    const ti=el('input',{class:'inp',placeholder:'e.g. Speech Session'});
    const si=el('input',{class:'inp',placeholder:'Specialist name'});
    const di=el('input',{class:'inp',type:'date','data-placeholder':'Select date'});
    if(prefilledDate) di.value=prefilledDate;
    const tmi=el('input',{class:'inp',placeholder:'e.g. 10:00 AM'});
    const typeSel=el('select',{class:'inp'});
    ['speech','ot','pt','general'].forEach(v=>{const o=el('option');o.value=v;o.textContent=v.toUpperCase();typeSel.appendChild(o);});

    // Share with specialists
    mb.appendChild(el('div',{class:'inp-label'},['Event type & sharing']));
    [ti,si,di,tmi,typeSel].forEach(i=>ig.appendChild(i));
    mb.appendChild(ig);

    const sharedWith=[];
    const shr=el('div',{class:'inp-group',style:{marginBottom:'16px'}});
    mb.appendChild(el('div',{class:'inp-label'},['Visible to specialists']));
    DB.specialists.filter(s=>s.status==='active').forEach(s=>{
      const lbl=el('label',{style:{display:'flex',alignItems:'center',gap:'9px',cursor:'pointer',padding:'8px 12px',borderRadius:'11px',background:'var(--mint-ll)'}});
      const cb=el('input',{type:'checkbox'});
    ...
    mb.appendChild(mkBtn('✅ Add Appointment','btn-lg btn-primary btn-full',()=>{
      ...
      const apts=DB.appointments;
      const specId=session?.specialistId||null;
      apts.push({id:'a_'+Date.now(),childId:S.activeChild,title:ti.value,specialist:si.value||session?.name||'TBD',specialistId:specId,date:di.value,time:tmi.value||'TBD',type:typeSel.value,sharedWith});
      DB.appointments=apts;
      if(DB.notifPrefs.appointments)pushNotif('report',`New appointment added: ${ti.value} on ${di.value}`,S.activeChild);
      close();re();toast('✅ Appointment added!');
      track('Appointment Added');
    }));
  },500);
}
...
function renderChat(){
  ...
  // FIX FS5: removed hardcoded 's1' fallback — a new specialist should only see their own chats
  const visChats=allChats.filter(c=>{
    if(role==='parent')return c.participants.includes('parent');
    if(!specId)return false; // no specialist ID means no accessible chats
    return c.participants.includes(specId);
  });

  const sec=el('div',{class:'section'});
  const hd=el('div',{class:'sec-hd'});
  hd.appendChild(el('h2',{class:'page-title'},['Messages']));
  hd.appendChild(el('div',{class:'sec-hd-right'},[
    session?.role==='parent'?mkBtn('👥 New Group Chat','btn-sm btn-secondary',()=>showCreateGroupChatModal()):el('span'),
    session?.role==='specialist'?mkBtn('🤝 Request Consult','btn-sm btn-lav',()=>showConsultModal()):el('span')
  ]));
  sec.appendChild(hd);

  const isMobile=window.innerWidth<768;
  const wrap=el('div',{class:'chat-layout',style:{display:'flex',flexDirection:isMobile?'column':'row',gap:'14px',minHeight:isMobile?'auto':'460px'}});

  // Chat list sidebar
  const list=el('div',{class:'chat-sidebar card',style:{padding:0,overflow:'hidden',width:isMobile?'100%':'200px',minWidth:isMobile?'unset':'200px',maxHeight:isMobile?'160px':'auto',overflowY:'auto',WebkitOverflowScrolling:'touch',borderRight:isMobile?'none':'1.5px solid var(--mint-l)'}});
  visChats.forEach(chat=>{
    ...
    const otherSpec=DB.specialists.find(s=>s.id===otherSpecId);
    const chatName=chat.name||(isConsult?'Specialist Consult':isGroup?'Group Chat':(otherSpec?.name||'Chat'));
    const childName=DB.children.find(c=>c.id===chat.childId)?.name||'';

    const item=el('div',{style:{padding:'13px 14px',borderBottom:'1.5px solid var(--mint-ll)',cursor:'pointer',background:S.activeChatId===chat.id?'var(--mint-l)':'#fff',transition:'background .12s',minHeight:'52px'}});
    item.innerHTML=`<div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
      ${isConsult?'<span class="badge badge-lav" style="font-size:.6rem;padding:1px 6px">Consult</span>':''}
      ${isGroup?'<span class="group-badge">Group</span>':''}
      <span style="font-weight:800;font-size:.82rem;color:var(--navy);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${chatName}</span></div>
      <div style="font-size:.68rem;color:var(--slate)">${childName}</div>
      <div style="font-size:.66rem;color:var(--slate-l);margin-top:1px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">${lastMsg?.text?.slice(0,28)||''}${(lastMsg?.text?.length||0)>28?'…':''}</div>`;
    item.onclick=()=>{S.activeChatId=chat.id;re();};
    list.appendChild(item);
  });
  if(!visChats.length){
    list.appendChild(el('div',{style:{padding:'20px 16px',color:'var(--slate)',fontSize:'.84rem',fontWeight:600}},['No chats yet.']));
  }
  wrap.appendChild(list);

  // Chat window
  const activeChat=allChats.find(c=>c.id===S.activeChatId);
  const win=el('div',{class:'chat-window card',style:{display:'flex',flexDirection:'column',padding:0,overflow:'hidden',flex:1,minHeight:isMobile?'360px':'auto'}});
  ...
  if(activeChat){
    ...
    const participantNames=isGroup
      ?activeChat.participants.map(p=>p==='parent'?'You':DB.specialists.find(s=>s.id===p)?.name?.split(' ')[0]||p).join(', ')
      :null;

    const winHd=d('',[], {padding:'13px 17px',borderBottom:'2px solid var(--mint-ll)',display:'flex',gap:'12px',alignItems:'center'});
    winHd.appendChild(mkAvatar(isGroup?'👥':isConsult?'🔗':'💬',36,'#d1fae5'));
    const wi=d('');
    wi.appendChild(el('div',{style:{fontWeight:800,fontSize:'.88rem',color:'var(--navy)'}},[activeChat.name||(otherSpec?.name||'Chat')]));
    wi.appendChild(el('div',{style:{fontSize:'.7rem',color:isGroup?'#0369a1':isConsult?'var(--lav-d)':'var(--teal)',fontWeight:700}},
      [isGroup?'👥 '+participantNames:isConsult?'🔗 Approved specialist consult':'🔒 Secure · Parent-approved']));
    winHd.appendChild(wi);
    win.appendChild(winHd);

    const msgArea=el('div',{id:'msg-area',style:{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',padding:'14px 16px',display:'flex',flexDirection:'column',gap:'11px',minHeight:'200px'}});
    activeChat.messages.forEach(m=>{
      // FIX FS5: use actual specId; if null, no messages should appear as "mine"
      const myId=role==='parent'?'parent':(specId||'');
      const isMe=m.sender===myId;
      const senderSpec=DB.specialists.find(s=>s.id===m.sender);
      const row=d('',[], {display:'flex',justifyContent:isMe?'flex-end':'flex-start'});
      const bub=el('div',{class:isMe?'bubble-me':'bubble-them'});
      if(!isMe)bub.appendChild(el('div',{class:'bubble-name'},[senderSpec?.name||(m.sender==='parent'?'Parent':'Unknown')]));
      bub.appendChild(el('div',{},[m.text]));
      bub.appendChild(el('div',{class:'bubble-time'},[m.time]));
      row.appendChild(bub);msgArea.appendChild(row);
    });
    win.appendChild(msgArea);
    // FIX F7: scroll after paint, not after an arbitrary timeout
    ...
    const mi=el('input',{class:'inp',placeholder:'Type a message…',style:{flex:'1',fontSize:'16px'}});
    mi.value=S.chatMsg||'';mi.oninput=e=>S.chatMsg=e.target.value;
    const sb2=el('button',{class:'send-btn',title:'Send message'},['➤']);
    sb2.onclick=()=>{
      const txt=mi.value.trim();if(!txt)return;
      const all=DB.chats,ci=all.findIndex(c=>c.id===activeChat.id);
      if(ci>-1){
        // FIX FS5: use actual specId; no fallback to 's1'
        const senderKey=role==='parent'?'parent':(specId||session?.email||'unknown');
        all[ci].messages.push({id:'m_'+Date.now(),sender:senderKey,text:txt,time:new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}),date:new Date().toISOString().split('T')[0]});track('Message Sent');
        DB.chats=all;
      }
      S.chatMsg='';re();
      // FIX F7: use requestAnimationFrame instead of setTimeout to scroll after DOM is rebuilt
      requestAnimationFrame(()=>{const ma=document.getElementById('msg-area');if(ma)ma.scrollTop=ma.scrollHeight;});
    };
    ...
    win.appendChild(el('div',{style:{display:'flex',alignItems:'center',justifyContent:'center',height:'100%',color:'var(--slate)',fontWeight:600}},['Select a conversation to begin']));
  }
...
function showConsultModal(){
  openModal('🤝 Request to Consult',(mb,close)=>{
    ...
    DB.specialists.filter(s=>s.id!==session?.specialistId&&s.status==='active').forEach(s=>{
      ...
      row.appendChild(info);
      row.appendChild(mkBtn('Request','btn-sm btn-primary',()=>{
        pushNotif('consult',`${spec?.name||'Specialist'} requested to consult with ${s.name} — awaiting your approval`,S.activeChild);
...
function renderVault(){
  ...
  if(activeNote){
    ...
    if(!activeNote.locked){
      ...
      // Save draft
      const saveDraftBtn=mkBtn('💾 Save Draft','btn-sm btn-slate',()=>{
      ...
      const lockBtn=mkBtn('🔒 Lock Report','btn-sm btn-ghost',()=>openConfirm('Lock Report?',
        'Locking makes this a permanent, uneditable record. This cannot be undone.',true,()=>{
          const ns=DB.vaultNotes,i=ns.findIndex(n=>n.id===activeNote.id);
    ...
    if(S.vaultSummarizing){
      ...
      if(!activeNote.locked&&!activeNote.published){
        const pubBtn=mkBtn('📤 Lock & Publish to Parent','btn-md btn-primary',()=>{
          openConfirm('Publish Summary?','Send this summary to the parent\'s profile and notify them?',false,()=>{
            const ns=DB.vaultNotes,i=ns.findIndex(n=>n.id===activeNote.id);
            if(i>-1){ns[i].published=true;ns[i].isDraft=false;DB.vaultNotes=ns;}
            pushNotif('report',`${DB.specialists.find(s=>s.id===specId)?.name||'Specialist'} published a new session summary for ${child?.name}`,S.activeChild);
...
function renderHomeworkParent(){
  ...
  hw.forEach(task=>{
    ...
    chk.onclick=()=>{
      ...
      if(i>-1){all[i].completed=!all[i].completed;DB.homework=all;
        if(!task.completed)pushNotif('homework',`"${task.task.slice(0,38)}…" marked complete ✅`,S.activeChild);
      }re();
    };
...
function renderHomeworkSpec(){
  ...
  hw.forEach(task=>{
    ...
    chk.onclick=()=>{
      const all=DB.homework,i=all.findIndex(h=>h.id===task.id);
      if(i>-1){all[i].completed=!all[i].completed;DB.homework=all;}re();
    };
    card.appendChild(chk);
    const info=d('',[], {flex:1});
...
function showAssignModal(){
  openModal('➕ Assign Homework Task',(mb,close)=>{
    ...
    mb.appendChild(mkBtn('✅ Assign Task','btn-lg btn-primary btn-full',()=>{
      ...
      const specId=session?.specialistId||null;
      const spec=DB.specialists.find(s=>s.id===specId);
      all.unshift({id:'h_'+Date.now(),childId:S.activeChild,task:ta.value.trim(),specialist:spec?.name||session?.name||'Specialist',specialistId:specId,due:di.value,completed:false,category:sel.value});
      DB.homework=all;
      pushNotif('homework',`New task: "${ta.value.slice(0,42)}…"`,S.activeChild);
...
function renderPatients(){
  ...
  ['active','inactive','all'].forEach(f=>{
    ...
    const btn=el('button',{class:f===statusFilter?'btn btn-sm btn-primary':'btn btn-sm btn-ghost'},[label]);
    btn.onclick=()=>{
      statusFilter=f;
  ...
  const doFilter=q=>{
    ...
    const filtered=sorted.filter(c=>{
      ...
      const matchStatus=statusFilter==='all'||(statusFilter===cs);
      return matchName&&matchStatus;
    });
    if(!filtered.length){
      const msg=statusFilter==='all'?'No patients found.':('No '+statusFilter+' patients.');
    ...
    filtered.forEach(c=>{
      ...
      card.innerHTML=`<div style="width:52px;height:52px;border-radius:15px;background:${c.color}44;display:flex;align-items:center;justify-content:center;font-size:1.8rem;flex-shrink:0">${c.avatar}</div>`;
      const info=d('',[], {flex:1});
      ...
      toggle.onclick=e=>{
        e.stopPropagation();
        const idx=DB.children.findIndex(ch=>ch.id===c.id);
        if(idx>-1){
          DB.children[idx].status=(c.status||'active')==='active'?'inactive':'active';
      ...
      card.onclick=()=>{S.activeChild=c.id;S.activeTab='vault';re();};
      stack.appendChild(card);
    });
  };
  doFilter('');
...
function renderCredentials(){
  const specId=session?.specialistId||null;
  const spec=DB.specialists.find(s=>s.id===specId)||{credentials:[]};
  const sec=el('div',{class:'section'});
  const hd=el('div',{class:'sec-hd'});
  hd.appendChild(el('div',{class:'sec-hd-left'},[
  ...
  if(!creds.length){
    ...
    creds.forEach(cr=>{
      const fi=el('div',{class:'file-item'});
        ...
        const specs=DB.specialists,i=specs.findIndex(s=>s.id===specId);
        if(i>-1){specs[i].credentials=specs[i].credentials.filter(c=>c.id!==cr.id);DB.specialists=specs;}
  ...
  const fi=el('input',{type:'file',style:{display:'none'}});
    ...
    openModal('📤 Upload Credential',(mb,close)=>{
      ...
      const upBtn=mkBtn('📤 Upload','btn-lg btn-primary btn-full',async()=>{
        ...
        try{
          const ab=await file.arrayBuffer();
          // FIX FS1: sanitize path segments for credential uploads too
          const sanitizeCredPath=s=>(s||'unknown').replace(/[\\/:*?"<>|]/g,'_').trim()||'unnamed';
          const resp=await fetch('https://content.dropboxapi.com/2/files/upload',{
            method:'POST',
            headers:{Authorization:'Bearer '+DROPBOX_TOKEN,'Content-Type':'application/octet-stream',
              'Dropbox-API-Arg':JSON.stringify({path:`/Huddledin/Specialists/${sanitizeCredPath(spec.name)}/Credentials/${sanitizeCredPath(file.name)}`,mode:'add',autorename:true})},body:ab
          });
          const data=await resp.json();
          const specs=DB.specialists,i=specs.findIndex(s=>s.id===specId);
          if(i>-1){
            if(!specs[i].credentials)specs[i].credentials=[];
            specs[i].credentials.push({id:'cr_'+Date.now(),name:data.name||file.name,date:new Date().toISOString().split('T')[0],type:typeSel.value});
            DB.specialists=specs;
          }
...
function renderNotifications(){
  const sec=el('div',{class:'section'});
  sec.appendChild(el('h2',{class:'page-title',style:{marginBottom:'20px'}},['Notifications']));
  const notifs=DB.notifications;
  if(!notifs.length){sec.appendChild(p('All caught up! 🎉'));return sec;}
  const stack=el('div',{class:'stack'});
  notifs.forEach(n=>{
    const card=el('div',{class:'card hov',style:{display:'flex',gap:'13px',alignItems:'flex-start',background:n.read?'#fff':'var(--mint-l)',border:n.read?'2px solid var(--mint-l)':'2.5px solid var(--teal-l)',cursor:'pointer'}});
    card.innerHTML=`<span style="font-size:1.45rem">${notifIcon[n.type]||'📢'}</span><div style="flex:1"><div style="font-weight:${n.read?600:800};color:var(--navy);line-height:1.4;margin-bottom:3px">${n.msg}</div><div style="font-size:.72rem;color:var(--slate-l)">${n.time}</div></div>`;
    if(!n.read)card.appendChild(badge('New','teal'));
    card.onclick=()=>{
      const ns=DB.notifications,i=ns.findIndex(x=>x.id===n.id);
      if(i>-1){ns[i].read=true;DB.notifications=ns;re();}
    };
    stack.appendChild(card);
  });
  sec.appendChild(stack);
...
function renderProgress(){
  ...
  const hd=el('div',{class:'sec-hd'});
  hd.appendChild(el('div',{class:'sec-hd-left'},[
    el('h2',{class:'page-title'},['📈 Progress Tracking']),
    el('p',{class:'page-sub'},[child?.avatar+' '+child?.name+' · Milestones & measurements'])
  ]));
  hd.appendChild(el('div',{class:'sec-hd-right'},[
    mkBtn('➕ Add Milestone','btn-sm btn-primary',()=>showAddMilestoneModal()),
    mkBtn('📊 Log Measurement','btn-sm btn-secondary',()=>showLogMeasurementModal())
  ]));
  sec.appendChild(hd);
  ...
  if(S.activeProgressTab==='overview'){
    ...
    if(pending.length){
      ...
      pending.forEach(m=>{
        ...
        acts.appendChild(mkBtn('✅ Mark Achieved','btn-sm btn-secondary',()=>{
          const ms=DB.milestones,i=ms.findIndex(x=>x.id===m.id);
          if(i>-1){ms[i].achieved=true;ms[i].date=new Date().toISOString().split('T')[0];DB.milestones=ms;}
          pushNotif('report','🏆 Milestone achieved: "'+m.title+'"',S.activeChild);re();toast('🏆 Milestone marked as achieved!');
        }));
    ...
    const stack=el('div',{class:'stack'});
    sorted.forEach(p=>{
      const spec=DB.specialists.find(s=>s.id===p.specialistId);
      const card=el('div',{class:'card hov',style:{display:'flex',gap:'13px',alignItems:'center'}});
...
function showAddMilestoneModal(){
  openModal('🏆 Add Milestone',(mb,close)=>{
    ...
    mb.appendChild(mkBtn('✅ Add Milestone','btn-lg btn-primary btn-full',()=>{
      ...
      ms.push({id:'ml_'+Date.now(),childId:S.activeChild,title:ti.value.trim(),date:di.value,achieved:false,category:catSel.value,specialistId:session?.specialistId||null});
      DB.milestones=ms;close();re();toast('✅ Milestone added!');
    }));
  },440);
}
...
function showLogMeasurementModal(){
  openModal('📊 Log Measurement',(mb,close)=>{
    ...
    mb.appendChild(mkBtn('📊 Save Measurement','btn-lg btn-primary btn-full',()=>{
      ...
      const ps=DB.progress;
      ps.push({id:'p_'+Date.now(),childId:S.activeChild,category:catSel.value,label:lb.value.trim(),value:parseFloat(vi.value),maxValue:parseFloat(mi2.value),unit:ui.value||'',date:di.value,specialistId:session?.specialistId||null});
      DB.progress=ps;close();S.activeProgressTab='overview';re();toast('📊 Measurement logged!');
    }));
  },460);
}
...
function showAddChildModal(){
  openModal('➕ Add Child Profile',(mb,close)=>{
    ...
    mb.appendChild(mkBtn('✅ Add Child','btn-lg btn-primary btn-full',async()=>{
      ...
      if(session&&session.id!=='demo'){try{const saved=await SB.addChild(session.id,{name:childName,dob:dob.value,avatar_emoji:selEmoji,color:'#c4b5fd',photo_url:null},session.householdId);if(saved)newChildId=saved.id;}catch(e){toast('Could not save: '+e.message,'error');return;}}
      const newChild={id:newChildId,name:childName,age,avatar:selEmoji,color:'#c4b5fd',dob:dob.value,photo:null};
...
function showChildPhotoModal(child){
  openModal('📷 Update '+child.name+"'s Photo",(mb,close)=>{
    ...
    photoInput.onchange=e=>{
      ...
      reader.onload=ev=>{
        photoData=ev.target.result;
        photoWrap.innerHTML=`<img src="${photoData}" class="photo-preview"/><div style="font-size:.78rem;color:var(--teal);font-weight:700;margin-top:6px">New photo selected ✓</div>`;
      };
      reader.readAsDataURL(f);
    };
    photoWrap.onclick=()=>photoInput.click();
...
function renderSpecialistRequest(){
  ...
  if(myRequests.length){
    ...
    myRequests.forEach(r=>{
      ...
      card.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px"><div style="font-weight:800;color:var(--navy)">${r.childName||'Unknown child'}</div>${r.status==='pending'?'<span class="badge badge-amber">⏳ Pending</span>':r.status==='approved'?'<span class="badge badge-green">✅ Approved</span>':'<span class="badge badge-rose">✗ Denied</span>'}</div>`;
  ...
  const ri=el('input',{class:'inp',placeholder:"Reason / your role (e.g. Speech Therapist for Lily)"});
...
function renderPublishedSummaries(){
  ...
  published.forEach(note=>{
    const spec=DB.specialists.find(s=>s.id===note.specialistId);
    ...
    const ta=el('textarea',{class:'inp',style:{background:'white',minHeight:'80px',fontSize:'.85rem'}});
    ta.value=note.summary;
    ta.readOnly=note.locked;
    ta.style.background=note.locked?'#f8fafc':'white';
    ta.oninput=e=>{
    ...
    nb.appendChild(ta);
    if(!note.locked){
      nb.appendChild(el('div',{style:{fontSize:'.72rem',color:'var(--teal)',fontWeight:700,marginTop:'6px'}},['✏️ You can annotate or add notes to this summary']));
    }
    wrap.appendChild(nb);
  });
  return wrap;
...
function showCreateGroupChatModal(){
  openModal('👥 Create Group Chat',(mb,close)=>{
    ...
    const activeSpecs=DB.specialists.filter(s=>s.status==='active');
    ...
    mb.appendChild(mkBtn('✅ Create Group Chat','btn-lg btn-primary btn-full',()=>{
      ...
      const participants=['parent',...Array.from(selected)];
      const chats=DB.chats;
      const newChat={id:'ch_'+Date.now(),type:'group',participants,childId:S.activeChild,approved:true,name,messages:[]};
      chats.push(newChat);DB.chats=chats;
      S.activeChatId=newChat.id;S.activeTab='chat';
...
function genHouseholdToken(){
  return 'hd_household_'+Math.random().toString(36).slice(2,8)+'_'+Math.random().toString(36).slice(2,10);
}
...
function renderSettings(){
  ...
  // Calendar sync
  const cc=el('div',{class:'card',style:{marginBottom:'16px'}});
  cc.appendChild(el('h3',{style:{fontWeight:800,color:'var(--navy)',marginBottom:'14px'}},['📅 Calendar Sync']));
  cc.appendChild(el('p',{style:{color:'var(--slate)',fontSize:'.86rem',marginBottom:'14px',lineHeight:'1.6',fontWeight:500}},['Connect your calendar to sync appointments automatically.']));
  const calBtns=d('',[], {display:'flex',gap:'10px',flexWrap:'wrap'});
  ['📅 Google Calendar','🍎 Apple iCal','📧 Outlook'].forEach(b=>{
    calBtns.appendChild(mkBtn(b,'btn-md btn-ghost',()=>showGoogleSyncModal()));
  });
  cc.appendChild(calBtns);sec.appendChild(cc);
...
(async function handleInviteLink(){
  console.log('handleInviteLink: search=',window.location.search,'hash=',window.location.hash);
  ...
  console.log('handleInviteLink: token=',token,'childId=',childId,'session=',session?.id);
  if(token){
    ...
    window.history.replaceState({},'',window.location.pathname);
    console.log('handleInviteLink: token stored in sessionStorage');
  ...
  console.log('handleInviteLink: pendingToken=',pendingToken,'session=',session?.id);
  if(pendingToken&&session){
    ...
    const retryInterval=setInterval(async()=>{
      retries++;
      console.log('handleInviteLink retry',retries,'session=',session?.id);
      if(session){
...
async function _processPendingInvite(){
  ...
  console.log('_processPendingInvite called, token:',token,'session:',session?.id);
  if(!token)return;
  // Try Supabase first
  ...
  console.log('Invite lookup result:', data?'found':'not found', fetchErr?JSON.stringify(fetchErr):'');
  if(data){
    if(new Date(data.expires_at)<new Date()){
      toast('⚠️ This invite has expired. Ask the family for a new one.','error',5000);
    } else {
      // Mark accepted
      const {error:upErr}=await _supa.from('invites').update({accepted:true,accepted_by:session.id}).eq('id',data.id);
      console.log('Invite marked accepted:', upErr?JSON.stringify(upErr):'ok');
      ...
      await _supa.from('specialists').upsert({
        ...
        status:'active'
      }).then(({error:e})=>{if(e)console.log('Specialists upsert error:',JSON.stringify(e));});
      ...
      if(data.child_id){
        ...
        console.log('Child lookup:', child?child.name:'not found');
        if(child){
          const children=LS.get('children',[]);
          if(!children.find(c=>c.id===child.id)){
            children.push({id:child.id,name:child.name,dob:child.dob,avatar:child.avatar_emoji||'🌸',color:child.color||'#c4b5fd',photo:child.photo_url||null});
            LS.set('children',children);
          }
