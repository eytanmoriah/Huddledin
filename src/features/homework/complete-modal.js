...
// Edit mode: when existingCompletion is passed, skip the status picker, hydrate
// note/photo/actual_value, save → UPDATE (not INSERT), expose Delete response button.

import { logExerciseCompletion, deleteExerciseCompletion } from './data.js';
import { injectHomeworkStyles } from './styles.js';

const T = (k, p) => window.HUD?.T?.(k, p) || k;

export function mountCompleteModal({ homework, exercise, slot, scheduledDate, childId, onSaved, existingCompletion }) {
  const isEdit = !!existingCompletion;
  ...
  // Attachment chips (per-exercise)
  {
    ...
    if (attachCount > 0) {
      ...
      for (let i = 0; i < attachCount; i++) {
        ...
        dlBtn.style.cssText = 'background:none;border:none;cursor:pointer;color:#64748b;padding:4px 6px;line-height:0;flex-shrink:0;font-family:inherit;display:inline-flex;align-items:center;justify-content:center;transition:color .12s;';
        ...
        dlBtn.onmouseenter = () => { dlBtn.style.color = '#0d9488'; };
        dlBtn.onmouseleave = () => { dlBtn.style.color = '#64748b'; };
        dlBtn.onclick = (e) => {
          e.stopPropagation();
  ...
  function renderForm(s) {
    ...
    // Back row + status pill (+ "Editing response" pill in edit mode)
    const backRow = document.createElement('div');
    backRow.style.cssText = 'display:flex;align-items:center;gap:8px;margin-bottom:14px;flex-wrap:wrap;';
    ...
    if (isEdit) {
      ...
      editPill.style.cssText = 'font-size:11px;font-weight:600;color:#7c3aed;background:#f5f3ff;border:1px solid #ddd6fe;border-radius:99px;padding:2px 8px;';
      editPill.textContent = T('hw4_edit_response');
      backRow.appendChild(editPill);
    }
    ...
    // Hydrate from existingCompletion only when current rendered status matches.
    // Status changes (e.g. Done → Couldn't): form starts fresh for the new status.
    const hydrate = isEdit && s.key === existingCompletion.status;
    ...
    if (s.key === 'done' && prescribed > 0) {
      ...
      actualValueInput.value = (hydrate && existingCompletion.actual_value != null)
        ? String(existingCompletion.actual_value)
        : String(prescribed);
    ...
    const saveLabel = isEdit ? T('hw4_update') : T('hw4_save');
    ...
    saveBtn.onclick = async () => {
      ...
      try {
        ...
        if (anyStale) {
          H.toast?.(T('hw4_response_stale'), 'error');
          close();
          onSaved?.();
          return;
        }
    ...
    // Delete response (edit-mode only)
    if (isEdit) {
      const delBtn = document.createElement('button');
      delBtn.style.cssText = 'width:100%;padding:10px 14px;border:1px solid #fecaca;border-radius:10px;background:#fff;color:#dc2626;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;margin-top:12px;';
      delBtn.textContent = T('hw4_delete_response');
      let isDeleting = false;
      delBtn.onclick = () => {
        if (isDeleting) return;
        H.openConfirm?.(T('hw4_delete_response'), T('hw4_delete_response_confirm'), true, async () => {
          if (isDeleting) return;
          isDeleting = true;
          delBtn.disabled = true;
          delBtn.textContent = T('btn_loading');
          try {
            await deleteExerciseCompletion(existingCompletion.id);
            close();
            H.toast?.(T('hw4_response_deleted'));
            onSaved?.();
          } catch (e) {
            console.error('❌ delete completion:', e);
            H.toast?.('Could not delete response.', 'error');
  ...
  if (isEdit) {
    const matching = statuses.find(st => st.key === existingCompletion.status);
    if (matching) renderForm(matching);
    else renderStatusPicker(); // defensive: unrecognized status
  } else {
    renderStatusPicker();
  }
