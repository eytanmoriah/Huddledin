...
export async function generatePDFBlob(reportText, reportType, childInfo, specialistInfo, branding) {
  ...
  doc.text(isHeb ? bidiLine(headerName) : headerName, pageW / 2, y, { align: 'center' });
  y += 7;
  if (brand.practice_address || brand.practice_phone || brand.practice_email) {
    doc.setFontSize(8);
    doc.setTextColor(100, 116, 139);
    const details = [brand.practice_address, brand.practice_phone, brand.practice_email].filter(Boolean).join(' · ');
    doc.text(isHeb ? bidiLine(details) : details, pageW / 2, y, { align: 'center' });
    y += 5;
  }
  doc.setFontSize(13);
